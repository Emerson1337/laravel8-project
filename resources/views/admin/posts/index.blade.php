...
@section('content')
<div class="grid">
    <a class="my-4 uppercase px-8 py-2 rounded bg-green-600 text-blue-50 max-w-max shadow-sm hover:shadow-lg" href="{{route('posts.create')}}">Cadastrar</a>
</div>
...
<form class="bg-white" action="{{route('posts.search')}}" method="post">
    @csrf
    <div class="max-w-sm my-4 p-1 pr-0 flex items-center">
        <input class="flex-1 appearance-none rounded shadow p-3 text-grey-dark mr-2 focus:outline-none" type="text" name="search" placeholder="Filtrar">
        <button class="uppercase p-3 rounded bg-blue-500 text-blue-50 max-w-max shadow-sm hover:shadow-lg" type="submit">Filtrar</button>
    </div>
</form>
